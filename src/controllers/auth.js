...
import { name, email, password, gender, phone, jobPosition, companyName, address, provinceCode, refreshToken } from "../helper/joi_schema";
import joi from 'joi'
...
export const employerRegister = async (req, res) =>{
    try {
        const { error } = joi.object({ name, email , password, gender, phone, jobPosition, companyName, address , provinceCode}).validate(req.body)
        if(error) return badRequest(error.details[0]?.message, res)
